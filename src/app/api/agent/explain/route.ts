...
export async function POST(req: Request) {
  ...
  if (!eventRes.ok) return NextResponse.json({ message: "EONET event fetch error" }, { status: 502 });
...
`.trim();
  ...
  // Si no vino JSON, devolvemos fallback para que el UI lo muestre igual.
